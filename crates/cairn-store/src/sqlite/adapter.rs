...
impl RunReadModel for SqliteAdapter {
    ...
    /// state. sqlite uses the same `idx_runs_parent` partial index
    /// as pg. `Running` is included so the driver can re-claim
...
struct RunRow {
    ...
    // approval-resume boundary. INTEGER NOT NULL DEFAULT 0 on the
    // schema (added alongside the other run-row columns).
    iteration: i64,
}
...
const RESOURCE_SHARE_SELECT_COLS_SQLITE: &str =
    "share_id, tenant_id, source_workspace_id, target_workspace_id, \
     resource_type, resource_id, permissions_json, shared_at_ms";

#[async_trait]
impl crate::projections::ResourceSharingReadModel for SqliteAdapter {
    async fn get_share(
        &self,
        share_id: &str,
    ) -> Result<Option<cairn_domain::resource_sharing::SharedResource>, StoreError> {
        let sql = format!(
            "SELECT {RESOURCE_SHARE_SELECT_COLS_SQLITE} FROM resource_shares
             WHERE share_id = ?"
        );
        let row: Option<SqliteResourceShareRow> = sqlx::query_as(&sql)
            .bind(share_id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        row.map(SqliteResourceShareRow::into_record).transpose()
    }
    ...
    async fn list_shares_for_workspace(
        ...
        let sql = format!(
            "SELECT {RESOURCE_SHARE_SELECT_COLS_SQLITE} FROM resource_shares
             WHERE tenant_id = ? AND target_workspace_id = ?
             ORDER BY shared_at_ms ASC, share_id ASC"
        );
        let rows: Vec<SqliteResourceShareRow> = sqlx::query_as(&sql)
            .bind(tenant_id.as_str())
            ...
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        rows.into_iter()
            .map(SqliteResourceShareRow::into_record)
            .collect()
    }
...
#[derive(sqlx::FromRow)]
struct SqliteSignalIngestionRow {
    signal_id: String,
    tenant_id: String,
    workspace_id: String,
    project_id: String,
...
const SIGNAL_INGESTION_SELECT_COLS_SQLITE: &str =
    "signal_id, tenant_id, workspace_id, project_id, source, payload_json, timestamp_ms";

#[async_trait]
impl crate::projections::SignalReadModel for SqliteAdapter {
    async fn get(
        &self,
        signal_id: &cairn_domain::SignalId,
    ) -> Result<Option<cairn_domain::SignalRecord>, StoreError> {
        let sql = format!(
            "SELECT {SIGNAL_INGESTION_SELECT_COLS_SQLITE} FROM signal_ingestions
             WHERE signal_id = ?"
        );
        let row: Option<SqliteSignalIngestionRow> = sqlx::query_as(&sql)
            .bind(signal_id.as_str())
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        row.map(SqliteSignalIngestionRow::into_record).transpose()
    }
    ...
    async fn list_by_project(
        ...
        let rows: Vec<SqliteSignalIngestionRow> = sqlx::query_as(&sql)
            .bind(project.tenant_id.as_str())
            .bind(project.workspace_id.as_str())
            .bind(project.project_id.as_str())
            .bind(limit as i64)
            ...
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        rows.into_iter()
            .map(SqliteSignalIngestionRow::into_record)
            .collect()
    }
}

// ── RFC-025 Phase 2b.2b m3: subagent_spawns read model ──

#[derive(sqlx::FromRow)]
struct SqliteSubagentSpawnRow {
...
impl crate::projections::SubagentSpawnReadModel for SqliteAdapter {
    ...
    async fn list_by_parent_run(
        ...
        let rows: Vec<SqliteSubagentSpawnRow> = sqlx::query_as(&sql)
            .bind(parent_run_id.as_str())
            .fetch_all(&self.pool)
            .await
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        Ok(rows
            .into_iter()
            .map(SqliteSubagentSpawnRow::into_record)
            .collect())
    }
}
...
const USER_MESSAGE_SELECT_COLS_SQLITE: &str =
    "run_id, sequence, tenant_id, workspace_id, project_id, \
     session_id, event_id, content, appended_at_ms";

#[async_trait]
impl crate::projections::UserMessageReadModel for SqliteAdapter {
    async fn list_by_run(
        &self,
        run_id: &cairn_domain::RunId,
        limit: usize,
        ...
        let sql = format!(
            "SELECT {USER_MESSAGE_SELECT_COLS_SQLITE} FROM user_messages
             WHERE run_id = ?
             ORDER BY sequence ASC, appended_at_ms ASC
             LIMIT ? OFFSET ?"
        );
        let rows: Vec<SqliteUserMessageRow> = sqlx::query_as(&sql)
            .bind(run_id.as_str())
            .bind(limit as i64)
            .bind(offset as i64)
            .fetch_all(&self.pool)
        ...
        Ok(rows
            .into_iter()
            .map(SqliteUserMessageRow::into_record)
            .collect())
    }

    async fn count_by_run(&self, run_id: &cairn_domain::RunId) -> Result<u64, StoreError> {
        let (count,): (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM user_messages WHERE run_id = ?")
                .bind(run_id.as_str())
                .fetch_one(&self.pool)
                .await
                .map_err(|e| StoreError::Internal(e.to_string()))?;
...
const SOUL_PATCH_SELECT_COLS_SQLITE: &str =
    "patch_id, tenant_id, workspace_id, project_id, state, patch_content, \
     requires_approval, proposed_at_ms, applied_at_ms, new_version";

#[async_trait]
impl crate::projections::SoulPatchReadModel for SqliteAdapter {
    async fn get(
        &self,
        patch_id: &str,
    ) -> Result<Option<crate::projections::SoulPatchRecord>, StoreError> {
        let sql =
            format!("SELECT {SOUL_PATCH_SELECT_COLS_SQLITE} FROM soul_patches WHERE patch_id = ?");
        let row: Option<SqliteSoulPatchRow> = sqlx::query_as(&sql)
            .bind(patch_id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        row.map(SqliteSoulPatchRow::into_record).transpose()
    }
    ...
    async fn list_by_project(
        ...
        let sql = format!(
            "SELECT {SOUL_PATCH_SELECT_COLS_SQLITE} FROM soul_patches
             WHERE tenant_id = ? AND workspace_id = ? AND project_id = ?
             ORDER BY proposed_at_ms DESC, patch_id DESC
             LIMIT ? OFFSET ?"
        );
        let rows: Vec<SqliteSoulPatchRow> = sqlx::query_as(&sql)
            .bind(project.tenant_id.as_str())
            .bind(project.workspace_id.as_str())
            .bind(project.project_id.as_str())
            .bind(limit as i64)
            ...
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        rows.into_iter()
            .map(SqliteSoulPatchRow::into_record)
            .collect()
    }
}
...
#[derive(sqlx::FromRow)]
struct SqliteToolRecoveryPauseRow {
    tool_call_id: String,
    tenant_id: String,
    workspace_id: String,
    project_id: String,
...
impl SqliteToolRecoveryPauseRow {
...
const TOOL_RECOVERY_PAUSE_SELECT_COLS_SQLITE: &str =
    "tool_call_id, tenant_id, workspace_id, project_id, run_id, task_id, \
     tool_name, reason, paused_at_ms";

#[async_trait]
impl crate::projections::ToolRecoveryPauseReadModel for SqliteAdapter {
    async fn get(
        &self,
        tool_call_id: &str,
    ) -> Result<Option<crate::projections::ToolRecoveryPauseRecord>, StoreError> {
        let sql = format!(
            "SELECT {TOOL_RECOVERY_PAUSE_SELECT_COLS_SQLITE} FROM tool_recovery_pauses
             WHERE tool_call_id = ?"
        );
        let row: Option<SqliteToolRecoveryPauseRow> = sqlx::query_as(&sql)
            .bind(tool_call_id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        Ok(row.map(SqliteToolRecoveryPauseRow::into_record))
    }
    ...
    async fn list_by_run(
        ...
        let rows: Vec<SqliteToolRecoveryPauseRow> = sqlx::query_as(&sql)
            .bind(run_id.as_str())
            .fetch_all(&self.pool)
            .await
            .map_err(|e| StoreError::Internal(e.to_string()))?;
